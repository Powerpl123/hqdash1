...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    ...
    WHERE id = user_uuid AND role IN ('system_admin', 'admin_officer')
  );
$$ LANGUAGE SQL SECURITY DEFINER STABLE;
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE id = user_uuid AND role IN ('system_admin', 'admin_officer', 'it_manager', 'it_officer', 'it_support')
  );
$$ LANGUAGE SQL SECURITY DEFINER STABLE;
...
-- Insert a default system admin user (you'll need to update this with a real user ID after authentication is set up)
INSERT INTO public.profiles (id, email, name, role, department, position, status)
VALUES (
